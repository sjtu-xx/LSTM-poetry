...
def load(poetry_file):
    ...
    poetrys = [poetry+END_CHAR for poetry in poetrys if (len(poetry)>5 and poetry[5]=="，")]

    print("收录{:d}首".format(len(poetrys)))
    words = []
    for poetry in poetrys:
        words.extend([word for word in poetry])
    ...
    unknown_char = char2id_dict[UNKNOWN_CHAR]
    char2id = lambda char:char2id_dict.get(char,unknown_char)
    # 由短到长排列
    poetrys = sorted(poetrys,key=lambda poetry:len(poetry))
    print(poetrys[:2])

    poetry_vector = [list(map(char2id,poetry)) for poetry in poetrys]
    return np.array(poetry_vector),char2id_dict,id2char_dict
...
#     poetry_vector,_,__ = load("poetry.txt")
#     print(poetry_vector[:2])